...
incidentController.show = function (req, res) {
    ...
    Incident.findOne({ id: id }).sort({ 'updates.updated': -1 }).exec(function (err, incidents) {
        if (err) {
...
incidentController.saveChild = function (req, res) {
    ...
    Incident.findOneAndUpdate({ id: req.params.id }, { $set: { status: req.body.status, workingteam: req.body.workingteam, priority: req.body.priority }, $push: { updates: cmnts } }, function (error, incidents) {
        if (error) {
            console.log(error);
        } else {
            //incident.save();
            res.redirect("/show/" + incidents.id);
        }
    });
};
...
incidentController.edit = function (req, res) {
    Incident.findOne({ id: req.params.id }).exec(function (err, incidents) {
        if (err) {
            console.log("Error:", err);
        }
        else {
...
incidentController.deleteMany = function (req, res) {
    ...
    Incident.deleteMany({ id: { $in: items } }, function (err, incidents) {
        if (err) {
            console.log(err);
        }
        else {
                ...
                if (err) {
                    ...
                    Incident.find({}).exec(function (err, incidents) {
                        if (err) {
                            ...
                            res.render("../views/index", { incidents: incidents });
                        }
                    });
                }
            });
...
incidentController.searchAAC = function (req, res) {
    ...
    Incident.find({ id: strId }).exec(function (err, incidents) {
        if (err) {
            console.log("Error:", err);
        }
        else {
